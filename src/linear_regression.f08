...
contains
  ...
  function predict(self, x) result(p)
    ...
    ! weights: w: (seq_len), b: scalar
    class(LinearRegression) :: self
    real(sp), intent (in) :: x(:, :)
    real(sp) :: p(size(x(1, :)))
    p = dot_product_last_axis(x, self%weights) + self%bias
  end function
  ...
  subroutine fit(self, x, y)
    ...
    real(sp), intent(in) :: x(:, :)
    real(sp), intent(in) :: y(:)
    real(sp) :: y_pred(size(x(1, :)))
    real(sp) :: dw(size(x(:, 1)))
    real(sp) :: db
    integer :: i
    do i = 1, self%iterations
      y_pred(:) = predict(self, x)
      dw = - (2 * dot_product_last_axis(transpose(x), y - y_pred)) / 100
      db = - (2 * sum(y - y_pred)) / 100
